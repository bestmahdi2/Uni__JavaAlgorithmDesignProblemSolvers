...
// used classes
import static com.ali.badiee.hw2_1.FractionalKnapsack.getMaxValue;
import static com.ali.badiee.hw2_1.FractionalKnapsack.items;
...
public class HW2Controller {
    ...
    void calculateFKP(ActionEvent event) {
        if (!capacityInput.getText().isEmpty() && isNumeric(capacityInput.getText()) && !itemSelection.getItems().isEmpty()) {
            resultLabel.setTextFill(Paint.valueOf("#50C878"));
            resultLabel.setText("Answer is > [ " + getMaxValue(Integer.parseInt(capacityInput.getText())) + " ]");
